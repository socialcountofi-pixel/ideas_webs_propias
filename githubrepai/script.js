...
async function updateStats(
  ...
  document.getElementById("totalPokemonColor").textContent =
    pokemon_color.count;
  document.getElementById("totalPokemonHabitat").textContent =
    pokemon_habitat.count; //cantidad al se array simple {}
  document.getElementById("totalPokemonShape").textContent =
    pokemon_shape.count;
  ...
  document.getElementById("totalEvolutionTrigger").textContent =
    evolution_trigger.count;
  document.getElementById("totalGrowthRate").textContent =
    growth_rate.count;
...
async function loadMenus() {
  try {
    ...
    const abilities = await fetch(`${API_BASE}/ability/?limit=10`).then((r) =>
      r.json(),
    );
    ...
    async function datosregion(s) {
     ...
     //console.log('abilitiesw', abilitiesw.locations);
      abilitiesw.locations.forEach((stat) => {
        //statsMenu.innerHTML += 'Movimiento Incrementa Stadist';
        //console.log('stats', stat);
        const option = document.createElement("option");
        option.href = "#";
      ...
      /* INCREASE */
     //console.log('abilitiesw', abilitiesw);
      abilitiesw.areas.forEach((stat) => { //console.log(stat);
        //statsMenu.innerHTML += 'Movimiento Incrementa Stadist';
        //console.log('stats', stat);
        const option = document.createElement("option");
        option.href = "#";
...
regionDaMenu.addEventListener("change", (e) => {
    ...
    const evolutiontriggerMenu = document.getElementById(
      "evolutiontriggerMenu",
    );
...
// growh rate
    ...
    const growthRrateMenu = document.getElementById(
      "growthrateMenu",
    );
    /* para que al pasa se use el typo de datos typeData.evolutiontrigger; que es especico de esta api */
    //console.log('params ', evolutiontrigger, 'paramsMenu ', evolutiontriggerMenu, ' ');
    growthRrate.results.forEach((stat) => {
      //console.log(' stat', stat);
      const link = document.createElement("a");
      link.href = "#";
      link.textContent = stat.name;
...
async function region_local_area(typeUrl) {
  ...
  for (const species of limited) {console.log(species.pokemon.name)
    const pokemon = await fetchPokemon(species.pokemon.name); //[{} {}]
 console.log(pokemon);
...
async function completados() {
  document.getElementById("generationsMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("generationsMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("typesMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("habilidadesMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("pokemoncolorMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("genderMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("pokemonshapeMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("pokemonhabitatMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("egggroupMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("evolutiontriggerMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  document.getElementById("pokedexMenu").style.backgroundColor =
    "rgba(255, 107, 107, 0.1)";
  /*
    document.getElementById('totalRegion').textContent = regions.count;
